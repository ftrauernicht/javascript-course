...
- **HTML** — die Struktur: welche Elemente es gibt (ein Button, eine
...
- **JavaScript** — das Verhalten: was passiert, wenn du klickst, tippst oder
...
die Elemente der Seite existieren, was wichtig ist: JavaScript, das läuft,
...
für sich allein testen, und genau deshalb lässt sie sich gleich so einfach
an Buttons anschließen. (Falls du diese Funktion schon in
...
- [`document.querySelector` / `querySelectorAll`](https://developer.mozilla.org/de/docs/Web/API/Document/querySelectorAll) findet Elemente über einen CSS-artigen Selektor — hier: "jedes Element mit einem `data-action`-Attribut gleich `"equals"`".
...
- `chooseOperator` behandelt außerdem Verkettung: Wird ein Operator
  ...
  `equals()` auf — genau das sorgt dafür, dass `3 + 4 × 2` korrekt `14`
...
Klammern und Rechenreihenfolge respektiert, genau das Problem, das die
optionale Klammer-Parser-Herausforderung in
[Kurs 2 – Taschenrechner Konsole](../../02-calculator-console/de/01-taschenrechner-konsole.md)
...
Klick `3`, `+`, `4`, `×`, `2`, `=` — du solltest Folgendes sehen:
