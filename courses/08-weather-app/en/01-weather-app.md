...
you type in. It's the first course where your code talks to the outside
world instead of only working with data you typed in yourself. This course
...
city, using the same "leave a container empty in HTML, fill it from
JavaScript" approach as every list or grid in earlier courses. See the
full version in [`index.html`](../code/index.html) and
...
These are given to you, pure DOM rendering using the same
`innerHTML`-from-data pattern as earlier courses, not the point of this
chapter:
